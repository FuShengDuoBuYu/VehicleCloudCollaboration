...
#-----------------------------------------------------------------------------------------
    def save_preds(self, vid_cap, idx, save_path):
        ...
        if self.dataset.mode == 'image':
            ...
            # --- 修正缩进后的核心逻辑 ---
            if color_mask1.shape[:2] != im0.shape[:2]:
                color_mask1 = cv2.resize(color_mask1, (im0.shape[1], im0.shape[0]))
            if color_mask2.shape[:2] != im0.shape[:2]:
                color_mask2 = cv2.resize(color_mask2, (im0.shape[1], im0.shape[0]))

            alpha = 0.5
            m1 = np.any(color_mask1 != [0, 0, 0], axis=-1)
            im0[m1] = (im0[m1] * (1 - alpha) + alpha * color_mask1[m1]).astype(np.uint8)

            m2 = np.any(color_mask2 != [0, 0, 0], axis=-1)
            im0[m2] = (im0[m2] * (1 - alpha) + alpha * color_mask2[m2]).astype(np.uint8)
            ...
            if self.vid_path[idx] != save_path:
                ...
                if vid_cap:
                    ...
                    w, h = int(vid_cap.get(3)), int(vid_cap.get(4))
                else:
